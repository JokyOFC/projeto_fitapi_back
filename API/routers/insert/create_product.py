...
@timeout(10)
@router.post("/create/produto/")
async def create_product(produto: Product):
    conn = None
    try:
        conn = await get_database_connection()

        values = (produto.nome, produto.descricao,
                  produto.codigo, produto.empresa_empresa_fk,
                  produto.preco, produto.is_Published,
                  produto.categoria_categoria_fk)
